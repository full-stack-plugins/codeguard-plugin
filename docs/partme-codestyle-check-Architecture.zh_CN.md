# partme-codestyle-check 系统架构设计

> **文档说明**：本架构文档描述 partme-codestyle-check 插件的内部结构、模块划分、数据流与三端适配设计。
>
> **版本**：V1.0
> **最后更新**：2026-09-17
...
partme-codestyle-check 是一类**约束型 AI 插件**——它不生产代码，而是在 AI 助手写代码的过程中**强制执行代码规范**。其核心创新点是把「linter 检查」从「提交/CI 阶段」**前移**到「AI 写完文件的瞬间」。
...
```mermaid
flowchart LR
    subgraph Plugin["partme-codestyle-check 插件"]
...
| 平台 | 安装方式 |
|---|---|
| ZCode | 软链或复制 `~/.zcode/plugins/partme-codestyle-check/` |
| Codex CLI | `~/.codex/plugins/` 或 marketplace install |
| Claude Code | 软链或复制 `~/.claude/plugins/partme-codestyle-check/` |
| Kimi Code | 软链或复制 `~/.kimi/plugins/partme-codestyle-check/` |
