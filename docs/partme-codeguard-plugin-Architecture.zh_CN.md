...
> **历史设计，不作为当前行为证明**：下文保存早期架构思路，其中 PostToolUse 阻断、javadoc 默认命令、延迟和“全覆盖”等叙述已不适用。当前模块职责见[架构及扩展指南](current-architecture.md)、[钩子协议](../hooks/__protocol__.md)及 OpenSpec；历史图表中的指标不是本次实测。
