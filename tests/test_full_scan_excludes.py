...
class FindExcludeInjectionTests(unittest.TestCase):
    ...
    def test_shell_gate_gains_target_exclude(self) -> None:
        ...
        self.assertIn("xargs -0 -r shellcheck --severity=warning", expr)
...
class JavadocShScenarioTests(unittest.TestCase):
    ...
    def test_no_shell_files_after_exclusion_passes(self) -> None:
        """GNU xargs 空输入仍会执行一次 shellcheck（exit 123）——gate 必须带 -r，
        否则"所有 .sh 都在构建目录里"的仓在 Linux 上永久误红。"""
        root = Path(tempfile.mkdtemp(prefix="cg-scan-"))
        gen = root / "target" / "apidocs"
        gen.mkdir(parents=True)
        (gen / "javadoc.sh").write_text("/usr/bin/javadoc @options @packages\n")
        self.assertEqual(self._gate_exit(root), 0)
