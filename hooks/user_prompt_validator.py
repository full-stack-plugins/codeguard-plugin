...
这是 codestyle-check 的「门禁钩子」——把规范检查从"提交后看 CI 红"前移到
"AI 在用户说提交那一刻就拦截"。
"""
...
def main() -> int:
    ...
    print(f"[codestyle-check] 检测到「提交/push」意图，运行 linter 门禁检查...")
    failures = []
    ...
    if failures:
        print(f"\n[codestyle-check] \u274c 门禁检查失败，请先修复:", file=sys.stderr)
        for lang, info in failures:
            print(f"  - {lang}: {info}", file=sys.stderr)
        print(f"\n[codestyle-check] 自动修复: python3 {PLUGIN_ROOT}/scripts/fix.py", file=sys.stderr)
        return 2

    print(f"[codestyle-check] \u2705 所有 linter 通过，可以提交")
    return 0
