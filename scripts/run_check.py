...
def cli_main():
    ...
    if not languages:
        print("[codestyle-check] 未识别到语言", file=sys.stderr)
        return 1

    print(f"[codestyle-check] project: {project_root}")
    print(f"[codestyle-check] languages: {languages}")
    print()
    ...
    if all(r["passed"] for r in results):
        print("[codestyle-check] \u2705 all passed")
        return 0
    print("[codestyle-check] \u274c some lints failed")
    return 2
...
def mcp_main(project_root: Path):
    ...
    print("[codestyle-check] MCP server mode not yet wired with official SDK.", file=sys.stderr)
    print("[codestyle-check] Use CLI mode for now: python3 run_check.py", file=sys.stderr)
    return 0
